...
import org.apache.james.jmap.json.Fixture.JMAP_CONFIGURATION
...
class SessionSupplierTest extends AnyWordSpec with Matchers {
  ...
  "generate" should {
    "return correct username" in {
      new SessionSupplier(JMAP_CONFIGURATION).generate(USERNAME).block().username should equal(USERNAME)
    }

    "return correct account" which {
      val accounts = new SessionSupplier(JMAP_CONFIGURATION).generate(USERNAME).block().accounts
